...
def init(config=None):
    ...
    #features_layers = ['output']

    graph_def_orig_file = '%s/../data/prereq/googlenet.pb' % os.path.dirname(os.path.realpath(__file__))
...
def model(x, googlenet):
    ...
    for op in reuse_ops:
        if is_early_loss(op.name):
            ...
            pool_op = tf.nn.avg_pool(T['mixed5b'], ksize=[1, 2, 2, 1],
                                     strides=[1, 1, 1, 1], padding='VALID', name=op.name)
            T[op.name] = pool_op
